...
public class SystemBarTintManagerUtils {
    ...
    private static boolean sIsMiuiSupport;
    ...
    static {
        try {
            ...
            sIsMiuiSupport = "V6".equals(version) || "V7".equals(version);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
     ...
     * @return
     */
    public static boolean setStatusBarDarkMode(boolean darkmode, Window window) {
        boolean flag = false;
        // miui
        if (sIsMiuiSupport) {
        ...
        // flyme
        else if (setStatusBarDarkIcon(window, darkmode)) {
            flag = true;
        }
        // API>=21 && darkmode=false
        else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT && !darkmode) {
            flag = true;
        }
        return flag;
    }

    public static boolean setStatusBarTintMode(Context context, boolean darkmode, Window window, int statusColorResouce) {
        if (setStatusBarDarkMode(darkmode, window)) {
            return setImmersedWindow(context, window, true, statusColorResouce);
        }
        return false;
    }
    ...
    public static boolean setImmersedWindow(Context context, Window window, boolean immersive, int statusColorResouce) {
        boolean result = false;
        if (window != null) {
            WindowManager.LayoutParams lp = window.getAttributes();
            int trans_status = 0;
            ...
            if (Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT) {
                ...
                window.setAttributes(lp);
                // window.getDecorView().setBackgroundResource(statusColorResouce);
                ((AppCompatActivity) context).getWindow().getDecorView().setBackgroundResource(statusColorResouce);
                // SystemBarTintManager mTintManager = new
                // SystemBarTintManager((Activity) context);
                // mTintManager.setStatusBarTintEnabled(true);
                // mTintManager.setStatusBarTintResource(statusColorResouce);
                result = true;
            } else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                window.clearFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS | WindowManager.LayoutParams.FLAG_TRANSLUCENT_NAVIGATION);
                window.getDecorView().setSystemUiVisibility(View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN | View.SYSTEM_UI_FLAG_LAYOUT_STABLE);
                window.addFlags(WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS);
                window.setStatusBarColor(context.getResources().getColor(statusColorResouce));
                result = true;
            }
        }
        return result;
