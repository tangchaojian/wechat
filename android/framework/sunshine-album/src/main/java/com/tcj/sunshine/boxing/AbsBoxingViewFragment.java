...
import com.tcj.sunshine.boxing.model.BoxingBuilderConfig;
...
import com.tcj.sunshine.boxing.utils.CameraPickerHelper;
...
public abstract class AbsBoxingViewFragment extends Fragment implements PickerContract.View {
     ...
     * call {@link #loadMedias()} or {@link #}, call {@link #loadAlbum()} if albums needed.
     */
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        com.tcj.sunshine.boxing.model.config.BoxingConfig config;
    ...
    private void initCameraPhotoPicker(Bundle savedInstanceState) {
        com.tcj.sunshine.boxing.model.config.BoxingConfig config = BoxingManager.getInstance().getBoxingConfig();
    ...
    public final void loadVideos() {
        mPresenter.loadVideos(0, AlbumEntity.DEFAULT_NAME);
    }
    ...
    public final void loadAudios() {
        mPresenter.loadAudios(0, AlbumEntity.DEFAULT_NAME);
    }
     ...
     * use {@link #showMedia(List, int)} to get the result.
     */
    public final void loadVideos(int page, String albumId) {
        mPresenter.loadVideos(page, albumId);
    }
     ...
     * use {@link #showMedia(List, int)} to get the result.
     */
    public final void loadAudios(int page, String albumId) {
        mPresenter.loadAudios(page, albumId);
    }
        ...
        try {
            if (!BoxingBuilderConfig.TESTING && ContextCompat.checkSelfPermission(getActivity(), CAMERA_PERMISSIONS[0]) != PERMISSION_GRANTED) {
                ...
                if (!BoxingManager.getInstance().getBoxingConfig().isVideoMode()) {
                    ...
                    mCameraPicker.startRecordVideo(activity, fragment, subFolderPath);
                }
            }
        } catch (IllegalArgumentException | IllegalStateException e) {
    ...
    private static final class CameraListener implements CameraPickerHelper.Callback {
        ...
        @Override
        public void onFinish(@NonNull CameraPickerHelper helper, String path) {
            ...
            if(TextUtils.isEmpty(path)) {
                file = new File(helper.getSourceFilePath());
            }else {
                file = new File(path);
            }
            ...
            if(index != -1) {
                ...
                if("mp4".equals(suffix.toLowerCase())) {
                    ...
                    cameraMedia.saveVideoStore(fragment.getAppCr(), file);
